...
bool MtcPickAndPlaceTask::buildTask()
{
    ...
    if (auto it = object_poses_.find(object_id_); it != object_poses_.end()) {
        ...
        RCLCPP_WARN(logger(), "[MtcTask:pick_and_place] No stored pose for '%s'; waist-aligned grasp assumes origin",
                    object_id_.c_str());
    }
    const double grasp_theta = std::atan2(object_pose.position.y, object_pose.position.x);
    if (waist_aligned) {
        RCLCPP_INFO(logger(), "[MtcTask:pick_and_place] %zu-DOF arm: waist-aligned grasp, azimuth=%.1f deg",
    ...
    // object<->other-object collisions stay checked.
